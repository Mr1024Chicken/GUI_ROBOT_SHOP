...
//	info.add_arm(Robot_arms("Iron Thunder 2100(lvl)","IRN_0025""Latest Iron Series Model!!!!",125,5,3));
//	info.add_arm(Robot_arms("Ice Breaker","ICE-2369","Freezzzing COOOOLLLLLL !!!!!",200,2,4));
//	info.add_arm(Robot_arms("Fist of God","GOD-2500","Gold Plated Fist Worthy For A GOD !!!!!",550,7,6));
//
//	info.add_battery(Robot_battery("Lithium ION-2000","ION-2530","Regular Battery",350,1,3));
//	info.add_battery(Robot_battery("Plutonium Charge","NCL-9999","Life-time Worth of Energy",5200,30,999999999));
//	info.add_battery(Robot_battery("Universal Charge","UNI-6531","Constant Universe Re-newable Energy",8000,15,999999999));
//
//	info.add_body(Robot_body("Knight-Ranger","KNR-5621","Vintage Knight Armor",1400,40,3));
//	info.add_body(Robot_body("Steel-Man","STL-6912","Industrial Product",1250,60,2));
//	info.add_body(Robot_body("Uncle_BOB","BOB-3030","Cheap Alternative",200,25,1));
//
...
//
//	info.add_motor(Robot_motor("V-8 Diesel Power Stroke","V8D-2500","Extreme Racing Engine",4000,60,1200,400));
//	info.add_motor(Robot_motor("2 Stroke 0.5L","2SK-6210","Slow But Steady Import Engine",500,20,35,30));
//	info.add_motor(Robot_motor("Electric Power Engine","EPE-5200","Clean Energy Racing Engine 0 Noise",8000,25,200,400));
//
//
//	return info;
//}
...
Robo Controller::add_arm(string name, string serial, string descripiton, int cost, int weight,int consumption,Robo info){
	info.add_arm(Robot_arms(name,serial,cost,weight,consumption,descripiton));
	return info;
}

Robo Controller::add_head(string name, string serial, int cost, int weight, string desc, Robo info){
	info.add_head(Robot_head(name,serial,cost,weight,desc));
	return info;
}


Robo Controller::add_torso(string name1,string serial1, string describe1, int cost, int weight, int slot,Robo info){
	info.add_body(Robot_body(name1,serial1,cost,weight,slot,describe1));
	return info;
}

Robo Controller::add_battery(string name, string serial, string describe, int price, int weight, int power, Robo info){
	info.add_battery(Robot_battery(name,serial,price,weight,power,describe));
	return info;
}

Robo Controller::add_motor(string name, string serial, string describe, int price, int weight, int consume, int speed, Robo info){
	info.add_motor(Robot_motor(name,serial,price,weight,consume,speed,describe));
	return info;
}

Robo Controller::add_model(string name, string serial, string describe, int arm_index, int head_index, int body_index, int motor_index, int battery_index,int price,int weight, Robo info){

	info.add_models(Robot_model(head_index,arm_index,body_index,motor_index,battery_index,name,serial,price,weight,describe));
	return info;
}

Robo Controller::add_customer(string name, string phone, Robo info){
	info.add_Customer(Customer(name,phone));
	return info;
}

Robo Controller::add_SA(string name, string ID, Robo info){
